...
{
    public partial class GenerateAsnFileEasy : BasePage
    {
        ...
        private string customerCodeEasy
        {
            get { return GetConst("CodeCustomerEasy")[0]; }
        }
        #endregion
        ...
        {
            context.SessionInfo.IdPage = "GenerateAsnFileEasy";
            ...
            {
                if (ValidateSession(WMSTekSessions.GenerateAsnFileEasy.DispatchList))
                {
                    dispatchSpecialViewDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileEasy.DispatchList];
                    isValidViewDTO = true;
                }
            }
        }
        ...
        protected void OpenPopUpASN(List<int> listIdDipatch)
        {
            ...
            try
            {
                ...
                {
                    var dispatchSpecialDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileEasy.DispatchList];
                ...
                if (!flagError)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileEasy.SelectedDispatchASN, listSpecialDispatch.FirstOrDefault());
                    Session.Add(WMSTekSessions.GenerateAsnFileEasy.ListIdDipatches, listIdDipatch);
        ...
        protected void btnGenerateASN_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidateSession(WMSTekSessions.GenerateAsnFileEasy.SelectedDispatchASN))
                {
                    var listIdDispatch = (List<int>)Session[WMSTekSessions.GenerateAsnFileEasy.ListIdDipatches];

                    ClearFilter("listDispatchDetail");
                    CreateFilterByList("listDispatch", listIdDispatch);

                    dispatchDetailViewDTO = iDispatchingMGR.GetByIdDispatchASNEasy("Easy", context);

                    if (!dispatchDetailViewDTO.hasError())
                    {
                        selecDispactch = (DispatchSpecial)Session[WMSTekSessions.GenerateAsnFileEasy.SelectedDispatchASN];
                        isValidViewDTO = true;
                        ...
                        if (dispatchDetailViewDTO.Entities != null && dispatchDetailViewDTO.Entities.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                if (dispatchDetailViewDTO.Entities.Exists(r=> string.IsNullOrEmpty(r.Item.ItemCustomer.ItemCodeCustomer)))
                                {
                                    string idDispatch = dispatchDetailViewDTO.Entities.First(r => string.IsNullOrEmpty(r.Item.ItemCustomer.ItemCodeCustomer)).Id.ToString();
                                    throw new Exception(lblDoesntExistItemCustomerMatch.Text + idDispatch);
                                }

                                var lstDispatchDetDist = (from p in dispatchDetailViewDTO.Entities
                                                     select new
                                                     {
        ...
        private void UpdateSession()
        {
            ...
            customerViewDTO = iWarehousingMGR.GetCustomerByCodeAndOwn(context, customerCodeEasy, int.Parse(ddlOwn.SelectedValue));

            if (customerViewDTO != null && customerViewDTO.Entities.Count > 0)
            {
                dispatchSpecialViewDTO = iDispatchingMGR.GetDispatchSpecialHeaderEasy(context, customerViewDTO.Entities.FirstOrDefault().Id);

                if (!dispatchSpecialViewDTO.hasError() && dispatchSpecialViewDTO.Entities != null)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileEasy.DispatchList, dispatchSpecialViewDTO);
                    isValidViewDTO = true;
        ...
        private void PopulateGridDetail()
        {
            ...
            {
                dispatchDetailViewDTO = (GenericViewDTO<DispatchDetail>)Session[WMSTekSessions.GenerateAsnFileEasy.DispatchDetailList];
        ...
        protected void LoadDetail(int index)
        {
            ...
            if (index != -1)
            {
                ...
                Session.Add(WMSTekSessions.GenerateAsnFileEasy.SelectedDispatchASN, dispatchSpecialViewDTO.Entities[index]);
                Session.Add(WMSTekSessions.GenerateAsnFileEasy.DispatchDetailList, dispatchDetailViewDTO);
        ...
        {
            ScriptManager.RegisterStartupScript(Page, GetType(), "gridDetail", "initializeGridDragAndDrop('Easy', 'ctl00_MainContent_hsMasterDetail_ctl00_ctl01_grdDetail');", true);
        }
    }
}