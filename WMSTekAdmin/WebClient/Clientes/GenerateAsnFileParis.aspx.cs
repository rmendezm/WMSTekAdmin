...
{
    public partial class GenerateAsnFileParis : BasePage
    {
        ...
        private string customerCodeParis
        {
            get { return GetConst("CodeCustomerParis")[0]; }
        }

        private string customerCodeJohnson
        {
            get { return GetConst("CodeCustomerJohnson")[0]; }
        }
        ...
        {
            context.SessionInfo.IdPage = "GenerateAsnFileParis";
            ...
            {
                if (ValidateSession(WMSTekSessions.GenerateAsnFileParis.DispatchList))
                {
                    dispatchSpecialViewDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileParis.DispatchList];
                    isValidViewDTO = true;
                }
            }
        }
        ...
        protected void OpenPopUpASN(List<int> listIdDipatch)
        {
            try
            {
                ...
                {
                    var dispatchSpecialDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileParis.DispatchList]; 
                ...
                if (!flagError)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileParis.SelectedDispatchASN, listSpecialDispatch.FirstOrDefault());
                    Session.Add(WMSTekSessions.GenerateAsnFileParis.ListIdDipatches, listIdDipatch);
        ...
        protected void btnGenerateASN_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidateSession(WMSTekSessions.GenerateAsnFileParis.SelectedDispatchASN))
                {
                    var listIdDispatch = (List<int>)Session[WMSTekSessions.GenerateAsnFileParis.ListIdDipatches];

                    ClearFilter("listDispatchDetail");
                    CreateFilterByList("listDispatch", listIdDispatch);

                    dispatchDetailViewDTO = iDispatchingMGR.GetByIdDispatchASNParis("Paris", context);

                    if (!dispatchDetailViewDTO.hasError())
                    {
                        selecDispactch = (DispatchSpecial)Session[WMSTekSessions.GenerateAsnFileParis.SelectedDispatchASN];
                        isValidViewDTO = true;
                        ...
                        if (dispatchDetailViewDTO.Entities != null && dispatchDetailViewDTO.Entities.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                dsToExcel(theData, Request.QueryString["CUSTOMER"].ToString().ToLower());
                            }
        ...
        private void UpdateSession()
        {
            ...
            if (Request.QueryString["CUSTOMER"] != null)
            {
                ...
                if (Request.QueryString["CUSTOMER"].ToString().ToUpper() == CustomerB2B.PARIS)
                {
                    custCode = customerCodeParis;
                }
                else if (Request.QueryString["CUSTOMER"].ToString().ToUpper() == CustomerB2B.JOHNSON)
                {
                    custCode = customerCodeJohnson;
                }

                customerViewDTO = iWarehousingMGR.GetCustomerByCodeAndOwn(context, custCode, int.Parse(ddlOwn.SelectedValue));

                if (customerViewDTO != null && customerViewDTO.Entities.Count > 0)
                {
                    dispatchSpecialViewDTO = iDispatchingMGR.GetDispatchSpecialHeaderParis(context, customerViewDTO.Entities.FirstOrDefault().Id);

                    if (!dispatchSpecialViewDTO.hasError() && dispatchSpecialViewDTO.Entities != null)
                    {
                        Session.Add(WMSTekSessions.GenerateAsnFileParis.DispatchList, dispatchSpecialViewDTO);
                        isValidViewDTO = true;
        ...
        private void PopulateGridDetail()
        {
            ...
            {
                dispatchDetailViewDTO = (GenericViewDTO<DispatchDetail>)Session[WMSTekSessions.GenerateAsnFileParis.DispatchDetailList];
        ...
        protected void LoadDetail(int index)
        {
            ...
            if (index != -1)
            {
                ...
                Session.Add(WMSTekSessions.GenerateAsnFileParis.SelectedDispatchASN, dispatchSpecialViewDTO.Entities[index]);
                Session.Add(WMSTekSessions.GenerateAsnFileParis.DispatchDetailList, dispatchDetailViewDTO);
        ...
        {
            ScriptManager.RegisterStartupScript(Page, GetType(), "gridDetail", "initializeGridDragAndDrop('Paris', 'ctl00_MainContent_hsMasterDetail_ctl00_ctl01_grdDetail');", true);
        }
    }
}