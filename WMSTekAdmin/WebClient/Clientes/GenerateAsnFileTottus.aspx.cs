...
{
    public partial class GenerateAsnFileTottus : BasePage
    {
        ...
        private string customerCodeTottus
        {
            get { return GetConst("CodeCustomerTottus")[0]; }
        }
        #endregion
        ...
        {
            context.SessionInfo.IdPage = "GenerateAsnFileTottus";
            ...
            {
                if (ValidateSession(WMSTekSessions.GenerateAsnFileTottus.DispatchList))
                {
                    dispatchSpecialViewDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileTottus.DispatchList];
                    isValidViewDTO = true;
                }
            }
        }
        ...
        protected void OpenPopUpASN(List<int> listIdDipatch)
        {
            ...
            try
            {
                ...
                {
                    var dispatchSpecialDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileTottus.DispatchList];
                ...
                if (!flagError)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileTottus.SelectedDispatchASN, listSpecialDispatch.FirstOrDefault());
                    Session.Add(WMSTekSessions.GenerateAsnFileTottus.ListIdDipatches, listIdDipatch);
        ...
        protected void btnGenerateASN_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidateSession(WMSTekSessions.GenerateAsnFileTottus.SelectedDispatchASN))
                {
                    var listIdDispatch = (List<int>)Session[WMSTekSessions.GenerateAsnFileTottus.ListIdDipatches];

                    ClearFilter("listDispatchDetail");
                    CreateFilterByList("listDispatch", listIdDispatch);

                    dispatchDetailViewDTO = iDispatchingMGR.GetByIdDispatchASNTottus("Tottus", context);

                    if (!dispatchDetailViewDTO.hasError())
                    {
                        selecDispactch = (DispatchSpecial)Session[WMSTekSessions.GenerateAsnFileTottus.SelectedDispatchASN];
                        isValidViewDTO = true;
                        ...
                        if (dispatchDetailViewDTO.Entities != null && dispatchDetailViewDTO.Entities.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                rowInbound["OC"] = separator + selecDispactch.OutboundOrder.ReferenceNumber;
                                rowInbound["FEC_DESPACHO"] = txtDateAsn.Text.Trim();
                                rowInbound["HORA_DESPACHO"] = txtHourAsn.Text.Trim();
                                rowInbound["CANT_LPN"] = int.Parse(dispatchDetailViewDTO.Entities.FirstOrDefault().SpecialField4);
                                ...
                                foreach (DispatchDetail disp in dispatchDetailViewDTO.Entities)
                                {
                                    ...
                                    rowDet["NRO_LINEA"] = "2";
                                    rowDet["COD_EAN"] = string.IsNullOrEmpty(disp.Item.ItemCustomer.BarCode) ? "" : disp.Item.ItemCustomer.BarCode;
                                    ...
                                    rowDet["TIPO_EMBALAJE"] = disp.Lpn.LPNType.Code;
                                ...
                                rowFooter["NRO_DOC_DESPACHO"] = selecDispactch.ReferenceDoc.ReferenceDocNumber;
                                ...
                                string attachment = "attachment; filename=epir-" + selecDispactch.ReferenceDoc.ReferenceDocNumber + "-" + DateTime.Now.ToString("ddMMyy-hhmm") + ".txt";
                                Response.Clear();
        ...
        private Dictionary<int, string> GetAllRowsSelected()
        {
            ...
            for (int i = 0; i < grdMgr.Rows.Count; i++)
            {
                ...
                {
                    listOutboundOrderSelected.Add(int.Parse(lblIdDispacth.Text.Trim()), lblReferenceDoc.Text.Trim());
                }
            }
        ...
        private void UpdateSession()
        {
            ...
            customerViewDTO = iWarehousingMGR.GetCustomerByCodeAndOwn(context, customerCodeTottus, int.Parse(ddlOwn.SelectedValue));

            if (customerViewDTO != null && customerViewDTO.Entities.Count > 0)
            {
                dispatchSpecialViewDTO = iDispatchingMGR.GetDispatchSpecialHeaderTottus(context, customerViewDTO.Entities.FirstOrDefault().Id);

                if (!dispatchSpecialViewDTO.hasError() && dispatchSpecialViewDTO.Entities != null)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileTottus.DispatchList, dispatchSpecialViewDTO);
                    isValidViewDTO = true;
        ...
        private void PopulateGridDetail()
        {
            ...
            {
                dispatchDetailViewDTO = (GenericViewDTO<DispatchDetail>)Session[WMSTekSessions.GenerateAsnFileTottus.DispatchDetailList];
        ...
        protected void LoadDetail(int index)
        {
            ...
            if (index != -1)
            {
                ...
                Session.Add(WMSTekSessions.GenerateAsnFileTottus.SelectedDispatchASN, dispatchSpecialViewDTO.Entities[index]);
                Session.Add(WMSTekSessions.GenerateAsnFileTottus.DispatchDetailList, dispatchDetailViewDTO);
        ...
        {
            ScriptManager.RegisterStartupScript(Page, GetType(), "gridDetail", "initializeGridDragAndDrop('Tottus', 'ctl00_MainContent_hsMasterDetail_ctl00_ctl01_grdDetail');", true);
        }
    }
}