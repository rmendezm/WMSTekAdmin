...
{
    public partial class GenerateAsnFileRipley : BasePage
    {
        ...
        private string customerCodeRipley
        {
            get { return GetConst("CodeCustomerRipley")[0]; }
        }
        #endregion
        ...
        {
            context.SessionInfo.IdPage = "GenerateAsnFileRipley";
            ...
            {
                if (ValidateSession(WMSTekSessions.GenerateAsnFileRipley.DispatchList))
                {
                    dispatchSpecialViewDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileRipley.DispatchList];
                    isValidViewDTO = true;
                }
            }
        }
        ...
        protected void OpenPopUpASN(List<int> listIdDipatch)
        {
            ...
            try
            {
                ...
                {
                    var dispatchSpecialDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileRipley.DispatchList];
                ...
                if (!flagError)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileRipley.SelectedDispatchASN, listSpecialDispatch.FirstOrDefault());
                    Session.Add(WMSTekSessions.GenerateAsnFileRipley.ListIdDipatches, listIdDipatch);
        ...
        protected void btnGenerateASN_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidateSession(WMSTekSessions.GenerateAsnFileRipley.SelectedDispatchASN))
                {
                    var listIdDispatch = (List<int>)Session[WMSTekSessions.GenerateAsnFileRipley.ListIdDipatches];

                    ClearFilter("listDispatchDetail");
                    CreateFilterByList("listDispatch", listIdDispatch);

                    dispatchDetailViewDTO = iDispatchingMGR.GetByIdDispatchASNRipley("Ripley", context);

                    if (!dispatchDetailViewDTO.hasError())
                    {
                        selecDispactch = (DispatchSpecial)Session[WMSTekSessions.GenerateAsnFileRipley.SelectedDispatchASN];
                        isValidViewDTO = true;
                        ...
                        if (dispatchDetailViewDTO.Entities != null && dispatchDetailViewDTO.Entities.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                rowInbound["NRO_DOC_DESPACHO"] = selecDispactch.ReferenceDoc.ReferenceDocNumber; //selecDispactch.OutboundOrder.Number;
                                rowInbound["DOC_ES_ELECTRONICO"] = "FACTURA_ELECTRONICA";
                                rowInbound["OC"] = selecDispactch.OutboundOrder.ReferenceNumber;
        ...
        private void UpdateSession()
        {
            ...
            customerViewDTO = iWarehousingMGR.GetCustomerByCodeAndOwn(context, customerCodeRipley, int.Parse(ddlOwn.SelectedValue));

            if (customerViewDTO != null && customerViewDTO.Entities.Count > 0)
            {
                dispatchSpecialViewDTO = iDispatchingMGR.GetDispatchSpecialHeaderRipley(context, customerViewDTO.Entities.FirstOrDefault().Id);

                if (!dispatchSpecialViewDTO.hasError() && dispatchSpecialViewDTO.Entities != null)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileRipley.DispatchList, dispatchSpecialViewDTO);
                    isValidViewDTO = true;
        ...
        private void PopulateGridDetail()
        {
            ...
            {
                dispatchDetailViewDTO = (GenericViewDTO<DispatchDetail>)Session[WMSTekSessions.GenerateAsnFileRipley.DispatchDetailList];
        ...
        protected void LoadDetail(int index)
        {
            ...
            if (index != -1)
            {
                ...
                Session.Add(WMSTekSessions.GenerateAsnFileRipley.DispatchDetailList, dispatchDetailViewDTO);
        ...
        private void dsToXml(DataSet ds)
        {
        ...
        {
            ScriptManager.RegisterStartupScript(Page, GetType(), "gridDetail", "initializeGridDragAndDrop('Ripley', 'ctl00_MainContent_hsMasterDetail_ctl00_ctl01_grdDetail');", true);
        }
    }
}