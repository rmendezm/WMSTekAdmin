...
{
    public partial class GenerateAsnFileFalabella : BasePage
    {
        ...
        private string customerCodeFalabella
        {
            get { return GetConst("CodeCustomerASNFalabella")[0]; }
        }
        #endregion
        ...
        {
            context.SessionInfo.IdPage = "GenerateAsnFileFalabella";
            ...
            {
                if (ValidateSession(WMSTekSessions.GenerateAsnFileFalabella.DispatchList))
                {
                    dispatchSpecialViewDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileFalabella.DispatchList];
                    isValidViewDTO = true;
                }
            }
        }
        ...
        protected void OpenPopUpASN(List<int> listIdDipatch)
        {
            ...
            try
            {
                ...
                {
                    var dispatchSpecialDTO = (GenericViewDTO<DispatchSpecial>)Session[WMSTekSessions.GenerateAsnFileFalabella.DispatchList];
                ...
                if (!flagError)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileFalabella.SelectedDispatchASN, listSpecialDispatch.FirstOrDefault());
                    Session.Add(WMSTekSessions.GenerateAsnFileFalabella.ListIdDipatches, listIdDipatch);
        ...
        protected void btnGenerateASN_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ValidateSession(WMSTekSessions.GenerateAsnFileFalabella.SelectedDispatchASN))
                {
                    var listIdDispatch = (List<int>)Session[WMSTekSessions.GenerateAsnFileFalabella.ListIdDipatches];

                    ClearFilter("listDispatchDetail");
                    CreateFilterByList("listDispatch", listIdDispatch);

                    dispatchDetailViewDTO = iDispatchingMGR.GetByIdDispatchASNFalabella("Falabella", context);

                    if (!dispatchDetailViewDTO.hasError())
                    {
                        selecDispactch = (DispatchSpecial)Session[WMSTekSessions.GenerateAsnFileFalabella.SelectedDispatchASN];
                        isValidViewDTO = true;
                        ...
                        if (dispatchDetailViewDTO.Entities != null && dispatchDetailViewDTO.Entities.Count > 0)
                        {
                            ...
                            else
                            {
                                ...
                                string nroFactura = (string.IsNullOrEmpty(selecDispactch.ReferenceDoc.ReferenceDocNumber) ? "0" : selecDispactch.ReferenceDoc.ReferenceDocNumber);
                                DataTable tableInbound = theData.Tables["PIR"];
                                DataRow rowInbound = tableInbound.NewRow();
                                rowInbound["num"] = selecDispactch.Id;
                                rowInbound["NRO_OC"] = selecDispactch.OutboundOrder.ReferenceNumber;
        ...
        private void UpdateSession()
        {
            ...
            customerViewDTO = iWarehousingMGR.GetCustomerByCodeAndOwn(context, customerCodeFalabella, int.Parse(ddlOwn.SelectedValue));

            if (customerViewDTO != null && customerViewDTO.Entities.Count > 0)
            {
                //dispatchSpecialViewDTO = iDispatchingMGR.GetDispatchSpecialHeader(context);
                dispatchSpecialViewDTO = iDispatchingMGR.GetDispatchSpecialHeaderFalabella(context, customerViewDTO.Entities[0].Id);

                if (!dispatchSpecialViewDTO.hasError() && dispatchSpecialViewDTO.Entities != null)
                {
                    Session.Add(WMSTekSessions.GenerateAsnFileFalabella.DispatchList, dispatchSpecialViewDTO);
                    isValidViewDTO = true;
        ...
        private void PopulateGridDetail()
        {
            ...
            {
                dispatchDetailViewDTO = (GenericViewDTO<DispatchDetail>)Session[WMSTekSessions.GenerateAsnFileFalabella.DispatchDetailList];
        ...
        protected void LoadDetail(int index)
        {
            ...
            if (index != -1)
            {
                ...
                Session.Add(WMSTekSessions.GenerateAsnFileFalabella.SelectedDispatchASN, dispatchSpecialViewDTO.Entities[index]);
                Session.Add(WMSTekSessions.GenerateAsnFileFalabella.DispatchDetailList, dispatchDetailViewDTO);
