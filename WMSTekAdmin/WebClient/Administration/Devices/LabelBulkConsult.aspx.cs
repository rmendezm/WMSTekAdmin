...
{
    public partial class LabelBulkConsult: BasePage
    {
        ...
        protected void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //labelTemplateViewDTO = iLabelMGR.GetLabelById(ddlLabelSize.SelectedValue.Trim(), context);
                    ...
                    foreach (GridViewRow row in this.grdMgr.Rows)
                    {
                        ...
                        if (chkbox.Checked)
                        {
                            ...
                            //Si el parametro se encuentra activo la tarea se genera con Estandar GS1
                            if (Convert.ToBoolean(Convert.ToInt16(GetCfgParameter("AllowGS1Standard"))) && ddlLabelSize.SelectedItem.Text.ToUpper().Contains("GS1"))
                            {
                                string typeLabel = ddlLabelSize.SelectedItem.Text.ToUpper().Contains("DATAMATRIX") ? "DATAMATRIX" : "128";

                                List<String> theGroupByList = new List<string>();
                                theGroupByList.Add("ItemCode");
                                theGroupByList.Add("ItemName");
                                theGroupByList.Add("LotNumber");
                                theGroupByList.Add("FabricationDate");
                                theGroupByList.Add("ExpirationDate");
                                theGroupByList.Add("ItemQty");

                                xmlParam = iLabelMGR.GetForShippingGS1(lblIdLpnCode.Trim(), theGroupByList, typeLabel, context);
                            }
                            else
                            {
                                xmlParam = iLabelMGR.GetForLPNDispachByIdLpn(lblIdLpnCode.Trim(), context);
                            }
