...
using System.Linq;
using rpt10 = Binaria.WMSTek.WebClient.ReportService2010;
using re05 = Binaria.WMSTek.WebClient.ReportExecution2005;

namespace Binaria.WMSTek.WebClient.Outbound.Consult
{
    public partial class PickingTaskConsultv2 : BasePage
    {
        ...
        protected override void Page_Init(object sender, EventArgs e)
        {
            try
            {
                ...
                if (base.webMode == WebMode.Normal)
                {
                    ...
                    if (ValidateSession(WMSTekSessions.PickingTaskMgrv2.PickingTaskList))
                    {
                        pickingViewDTO = (GenericViewDTO<TaskConsult>)Session[WMSTekSessions.PickingTaskMgrv2.PickingTaskList];
                        isValidViewDTO = true;
                    }
        ...
        protected void SaveChanges(int index)
        {
            ...
            var tasks = (GenericViewDTO<TaskConsult>)Session[WMSTekSessions.PickingTaskMgrv2.PickingTaskList];

            var taskById = tasks.Entities.Where(t => t.Id == task.Id).FirstOrDefault();
            var outboundNumber = string.Empty;

            if (taskById != null)
            {
                if (HasChangedPriorityOrAndelLocation(taskById, task))
                {
                    var idOutboundOrder = taskById.Task.IdDocumentBound;
                    var getOutbounOrder = iDispatchingMGR.GetOutboundByAnyParameter(context, new OutboundOrder() { Id = idOutboundOrder }, null);

                    if (!getOutbounOrder.hasError() && getOutbounOrder.Entities.Count > 0)
                        outboundNumber = getOutbounOrder.Entities.First().Number;
                }
            }
            ...
            else
            {
                ...
                {
                    var outboundNumberList = new List<string>() { outboundNumber };
                    CallRptDispatchVolumneDPS(outboundNumberList, "DPS");
                }
            }
        }
        private bool HasChangedPriorityOrAndelLocation(TaskConsult originalTask, Task modifiedTask)
        {
            return originalTask.Task.Priority != modifiedTask.Priority || originalTask.IdLocStageTarget != modifiedTask.StageTarget.IdCode;
        }
        #endregion
        ...
        private void UpdateSession(bool showError)
        {
            ...
            {
                Session.Add(WMSTekSessions.PickingTaskMgrv2.PickingTaskList, pickingViewDTO);
                isValidViewDTO = true;
        ...
        private void CallRptDispatchVolumneDPS(List<string> outboundNumber, string prefixReport)
        {
            try
            {
                ReportExecution2005.ParameterValue[] executionParams = new ReportExecution2005.ParameterValue[1];
                executionParams[0] = new ReportExecution2005.ParameterValue();
                executionParams[0].Name = "OutboundNumberOrder";
                executionParams[0].Value = "'" + string.Join("','", outboundNumber.ToArray()) + "'";

                var reportDispatchVolume_DPSPath = GetConst("ReportDispatchVolume_DPSPath")[0];

                var results = CallRPTBase(reportDispatchVolume_DPSPath, prefixReport, executionParams);

                SendRptToClient(results, prefixReport + DateTime.Now.ToString("dd-MM-yyyy HHmmss") + ".pdf");
            }
            catch (Exception ex)
            {
                pickingViewDTO.Errors = baseControl.handleException(ex, context);
        ...
        private byte[] CallRPTBase(string reportPath, string prefixReport, ReportExecution2005.ParameterValue[] executionParams)
        {
            try
            {
                if (executionParams != null && executionParams.Count() > 0)
                {
                    re05.ReportExecutionService rsExec = new re05.ReportExecutionService();
                    rsExec.Credentials = System.Net.CredentialCache.DefaultCredentials;

                    var reportingServiceAddress = GetConst("ReportingServiceAddress")[0];
                    var wsReportExecutionService = GetConst("WsReportExecutionService")[0];

                    rsExec.Url = reportingServiceAddress + wsReportExecutionService;
                    ...
                    string encoding;
                    string mimeType;
                    re05.Warning[] warnings = null;
                    string[] streamIDs = null;
                    string format = "PDF";

                    return rsExec.Render(format, deviceInfo, out extension, out mimeType, out encoding, out warnings, out streamIDs);
                }
                else
                {
                    throw new ArgumentNullException("Parametros del reporte inválidos, son null");
                }
            }
            catch (Exception ex)
            {
        ...
        private void SendRptToClient(byte[] rptAsByteArray, string rptName)
        {
            try
            {
                Session["DataRpt"] = rptAsByteArray;
                string url = ResolveUrl("~/DownloadDocument.ashx?filename=" + rptName);
                ScriptManager.RegisterStartupScript(Page, GetType(), "callIframe", "callIframe(\"" + url + "\");", true);
            }
            catch (Exception ex)
            {
                throw ex;
