...
{
    public partial class PackageTaskConsult : BasePage
    {
            ...
            try
            {
                ...
                if (base.webMode == WebMode.Normal)
                {
                    ...
                    else
                    {
                        ...
                        if (ValidateSession(WMSTekSessions.PackageTaskMgr.PackageTaskList))
                        {
                            pickingViewDTO = (GenericViewDTO<TaskConsult>)Session[WMSTekSessions.PackageTaskMgr.PackageTaskList];
                            isValidViewDTO = true;
                        }
        ...
        protected void ShowModal(int index, CRUD mode)
        {
            ...
            {
                if (mode == CRUD.Create)
                    base.ConfigureModal(pickingViewDTO.Configuration, true);
                else
                {
                    base.ConfigureModal(pickingViewDTO.Configuration, false);  
                }
        ...
        private void InitializeFilter(bool init, bool refresh)
        {
            ...
            this.Master.ucMainFilter.listTaskTypeCode = GetConst("PackageTaskConsult");
        ...
        private void UpdateSession(bool showError)
        {
            ...
            {
                // carga las tareas
                pickingViewDTO = iTasksMGR.FindAllPackageTask(context);
            }
            else
            {
                // carga las tarea Detalle
                pickingViewDTO = iTasksMGR.FindAllPackageDetailTask(context);
            }

            if (!pickingViewDTO.hasError() && pickingViewDTO.Entities != null)
            {
                Session.Add(WMSTekSessions.PackageTaskMgr.PackageTaskList, pickingViewDTO);
                isValidViewDTO = true;
