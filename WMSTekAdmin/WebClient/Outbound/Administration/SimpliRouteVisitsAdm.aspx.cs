...
using Binaria.WMSTek.DataAccess.Dispatching;
...
{
    public partial class SimpliRouteVisitsAdm : BasePage
    {
        #region "Declaración de Variables"

        private GenericViewDTO<SimpliRouteVisit> simpliRouteVisitViewDTO = new GenericViewDTO<SimpliRouteVisit>();
        private GenericViewDTO<SimpliRouteVisitConsult> SimpliRouteVisitConsultViewDTO = new GenericViewDTO<SimpliRouteVisitConsult>();
        ...
        protected override void Page_Init(object sender, EventArgs e)
        {
            ...
            {
                simpliRouteVisitViewDTO.Errors = baseControl.handleException(ex, context);
                this.Master.ucError.ShowError(simpliRouteVisitViewDTO.Errors);
            }
        }
            ...
            try
            {
                ...
                if (e.Row.RowType == DataControlRowType.DataRow)
                {
                    ...
                    if (btnCancelTaskQueue != null)
                    {
                        ...
                        if (simpliRouteVisitViewDTO.Entities.Count > 0 && simpliRouteVisitViewDTO.Entities[e.Row.DataItemIndex].TrackTaskQueue.IdTrackTaskQueue == (int)eTrackTaskQueue.Waiting)
                        {
                    ...
                    if (btnRetryTaskQueue != null)
                    {
                        ...
                        if (simpliRouteVisitViewDTO.Entities.Count > 0 && (simpliRouteVisitViewDTO.Entities[e.Row.DataItemIndex].TrackTaskQueue.IdTrackTaskQueue == (int)eTrackTaskQueue.Error || simpliRouteVisitViewDTO.Entities[e.Row.DataItemIndex].TrackTaskQueue.IdTrackTaskQueue == (int)eTrackTaskQueue.Cancel))
                        {
        ...
        protected void grdMgr_RowEditing(object sender, GridViewEditEventArgs e)
        {
            /*
                       try
                       {
                           ...
                           simpliRouteVisitViewDTO = (GenericViewDTO<TaskQueue>)Session[WMSTekSessions.SimpliRouteVisitAdm.List];
                           var taskQueueToRetry = simpliRouteVisitViewDTO.Entities[retryIndex];
        ...
        protected void grdMgr_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            try
            {
                ...
                simpliRouteVisitViewDTO = (GenericViewDTO<TaskQueue>)Session[WMSTekSessions.SimpliRouteVisitAdm.List];
                var taskQueueToCancel = simpliRouteVisitViewDTO.Entities[cancelIndex];
        ...
        protected void btnApiDetail_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var taskQueueSelected = (TaskQueue)Session[WMSTekSessions.SimpliRouteVisitAdm.Selected];
    ...
    */
        ...
        protected void ucStatus_pageChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                this.lblGridDetail.Visible = false;
                PopulateGrid();
            }
            catch (Exception ex)
            {
        ...
        {
            context.SessionInfo.IdPage = "SimpliRouteVisitsAdm";
            ...
            {
                if (ValidateSession(WMSTekSessions.SimpliRouteVisitAdm.List))
                {
                    simpliRouteVisitViewDTO = (GenericViewDTO<SimpliRouteVisit>)Session[WMSTekSessions.SimpliRouteVisitAdm.List];
                    isValidViewDTO = true;
                }
        ...
        private void InitializeFilter(bool init, bool refresh)
        {
            ...
            this.Master.ucMainFilter.idTruckCodeVisible = true;
            this.Master.ucMainFilter.codeVisible = true;
            this.Master.ucMainFilter.codeLabel = this.lblCodeLpn.Text;
            this.Master.ucMainFilter.simpliRouteVisitStatusVisible = true;
        ...
        private void PopulateGrid()
        {
            ...
            if (!simpliRouteVisitViewDTO.hasConfigurationError() && simpliRouteVisitViewDTO.Configuration != null && simpliRouteVisitViewDTO.Configuration.Count > 0)
                base.ConfigureGridOrder(grdMgr, simpliRouteVisitViewDTO.Configuration);

            grdMgr.DataSource = simpliRouteVisitViewDTO.Entities;
            grdMgr.DataBind();

            CallJsGridViewHeader();

            ucStatus.ShowRecordInfo(simpliRouteVisitViewDTO.Entities.Count, grdMgr.PageSize, grdMgr.PageCount, currentPage, grdMgr.AllowPaging);
        }
        private void PopulateGridDetail()
        {
            ...
            {
                dispatchDetailViewDTO = (GenericViewDTO<DispatchDetail>)Session[WMSTekSessions.SimpliRouteVisitAdm.Detail];
        ...
        protected void btnExcelDetail_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (index != -1)
                {
                    ...
                    string idCode = simpliRouteVisitViewDTO.Entities[index].IdLpnCode;
                    ...
                    CreateFilterByList("IdLpnCode", new List<string>() { idCode });

                    GenericViewDTO<TaskQueue> taskQueueViewDTO = new GenericViewDTO<TaskQueue>();
                    ...
                    ////    if (!stockViewDTO.hasError() && stockViewDTO.Entities.Count > 0)
                    {
                        ...
                        dispatchDetailViewDTO = iDispatchingMGR.GetLpnsForCourierByOutboundOrder(taskQueueViewDTO.Entities[0].IdDocumentBound/*stockViewDTO.Entities[0].OutboundOrder.Id*/, context);

                        //stockViewDTO = iDispatchingMGR.GetDetailPackageByIdLpnCode(idCode, context);
                        this.lblNroDoc.Text = (taskQueueViewDTO.Entities[0].IdDocumentBound.ToString());
                        ...
                        if (stockViewDTO != null && stockViewDTO.Entities.Count > 0)
                        {
                            ...
                            // Detalle 
                            List<SimpliRouteVisit> listTemp = new List<SimpliRouteVisit>();
                             listTemp.Add(simpliRouteVisitViewDTO.Entities[index]);
        ...
        protected void LoadDetail(int index)
        {
            ...
            if (index != -1)
            {
                ...
                stockViewDTO = iDispatchingMGR.GetDetailPackageByIdLpnCode(idCode, context);
                this.lblNroDoc.Text = (simpliRouteVisitViewDTO.Entities[index].IdLpnCode);
                this.lblGridDetail.Visible = true;
            ...
            */
            ...
            if (index != -1)
            {
                ...
                */
                //string idCode = simpliRouteVisitViewDTO.Entities[index].id;
                ...
                CreateFilterByList("IdTypeTask", new List<int>() { (int)eTypeTaskQueue.SimplirouteIntegration });
                ClearFilter("LpnCode");
                CreateFilterByList("LpnCode", new List<string>() { idCode });
                ...
                if (dispatchDetailViewDTO != null && dispatchDetailViewDTO.Entities.Count > 0)
                {
                    ...
                    try
                    {
                        GenericViewDTO<OutboundOrder> outboundOrder = outboundOrderDAO.GetById(stockViewDTO.Entities[0].OutboundOrder.Id);
                        if(!outboundOrder.hasError() && outboundOrder.Entities.Count > 0)
                            this.lblNroDoc.Text = outboundOrder.Entities[0].Number.ToString();
                    }
                    catch { }
                    ...
                    /*
                    switch ((eTrackTaskQueue)taskQueueViewDTO.Entities[0].TrackTaskQueue.IdTrackTaskQueue)//.TrackTaskQueue.IdTrackTaskQueue)
                    {
                        case eTrackTaskQueue.InProcess:
                        ...
                        case eTrackTaskQueue.Successful:
                            this.lblStatusDescription.Text = "Tarea De Carga Exitosamente Creada";
                            break;
                        case eTrackTaskQueue.SecondStepSuccessful:
        ...
        private void UpdateSession()
        {
            ...
            SimpliRouteVisit theSimpliRouteVisit = new SimpliRouteVisit();
            SimpliRouteVisitConsult theSimpliRouteVisitConsult = new SimpliRouteVisitConsult();

            // carga consulta de Lpn
            TextBox txtLpnCode = (TextBox)this.Master.ucMainFilter.FindControl("txtFilterCode");
            string lpnFilter = txtLpnCode.Text;
            string truckFilter = this.Master.ucMainFilter.idTruckCode;

            /*
            if (!string.IsNullOrEmpty(truckFilter))
            { theSimpliRouteVisit.IdTruckCode = truckFilter; }
            if (!string.IsNullOrEmpty(lpnFilter))
            {
                theSimpliRouteVisit.IdLpnCode = lpnFilter;
            }*/
            ClearFilter("IdTruckCode");
            CreateFilterByList("IdTruckCode", new List<string>() { truckFilter});

            //theSimpliRouteVisit.IdTruckCode = this.txtIdTruckCode.Text;
            simpliRouteVisitViewDTO = iDispatchingMGR.SimpliRouteVisitGetByAnyParameter( context);
            //SimpliRouteVisitConsultViewDTO = iDispatchingMGR.SimpliRouteVisitConsultGetByAnyParameter(theSimpliRouteVisitConsult, context);

            if (!simpliRouteVisitViewDTO.hasError() && simpliRouteVisitViewDTO.Entities != null)
            {
                Session.Add(WMSTekSessions.SimpliRouteVisitAdm.List, simpliRouteVisitViewDTO);
                isValidViewDTO = true;

                ucStatus.ShowMessage(simpliRouteVisitViewDTO.MessageStatus.Message);
            }
            else
            {
                if (simpliRouteVisitViewDTO.Errors.Code != WMSTekError.DataBase.NoRowsReturned)
                {
                    this.Master.ucError.ShowError(simpliRouteVisitViewDTO.Errors);
                }
                else
                {
                    Session.Remove(WMSTekSessions.SimpliRouteVisitAdm.List);
                }
            }
