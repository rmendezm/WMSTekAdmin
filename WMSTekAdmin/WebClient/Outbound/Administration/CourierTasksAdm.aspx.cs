...
{
    public partial class CourierTasksAdm : BasePage
    {
        ...
        protected void grdMgr_RowEditing(object sender, GridViewEditEventArgs e)
        {
            try
            {
                ...
                taskQueueViewDTO = (GenericViewDTO<TaskQueue>)Session[WMSTekSessions.CourierTasksAdm.List];
        ...
        protected void btnApiDetail_Click(object sender, EventArgs e)
        {
            try
            {
                var taskQueueSelected = (TaskQueue)Session[WMSTekSessions.CourierTasksAdm.Selected];

                if (taskQueueSelected != null)
                {
                    var EnviameApiUrl = ConfigurationSettings.AppSettings["EnviameApiUrl"];
                    var url = EnviameApiUrl + "deliveries/" + taskQueueSelected.SpecialField2 + "/tracking";
                    string key = ConfigurationSettings.AppSettings["EnviameApiKey"];

                    var details = iDispatchingMGR.ShipmentDetail(url, key);

                    txtStatusName.Text = details.data.status.name;
                    txtCarrier.Text = details.data.carrier;
                    txtCustomer.Text = details.data.customer.full_name;
                    txtAddress.Text = details.data.shipping_address.full_address;
                    txtCity.Text = details.data.shipping_address.place;
                    linkLabel.NavigateUrl = details.data.label.PDF;
                    ...
                    grdTracking.DataSource = details.data.tracking;
        ...
        private void InitializeFilter(bool init, bool refresh)
        {
            ...
            //this.Master.ucMainFilter.date = true;
        ...
        private void PopulateGridDetail()
        {
            ...
            {
                dispatchDetailViewDTO = (GenericViewDTO<DispatchDetail>)Session[WMSTekSessions.CourierTasksAdm.Detail];
        ...
        protected void LoadDetail(int index)
        {
            if (index != -1)
            {
                ...
                Session.Add(WMSTekSessions.CourierTasksAdm.Selected, taskQueueSelected);
                Session.Add(WMSTekSessions.CourierTasksAdm.Detail, dispatchDetailViewDTO);

                this.lblNroDoc.Text = taskQueueSelected.NumberDocumentBound;

                if (taskQueueSelected.TrackTaskQueue.IdTrackTaskQueue == (int)eTrackTaskQueue.FirstStepSuccessful ||
        ...
        private void UpdateSession()
        {
            ...
            CreateFilterByList("IdTypeTask", new List<int>() { (int)eTypeTaskQueue.CarrierIntegration });
            ...
            {
                Session.Add(WMSTekSessions.CourierTasksAdm.List, taskQueueViewDTO);
                isValidViewDTO = true;
            ...
            else
            {
                ...
                {
                    Session.Remove(WMSTekSessions.CourierTasksAdm.List);
                }
            }
        }
