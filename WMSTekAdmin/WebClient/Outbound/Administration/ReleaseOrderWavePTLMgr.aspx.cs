...
{
    public partial class ReleaseOrderWavePTLMgr : BasePage
    {
        ...
        protected void btnReprocess_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                if (ValidateSession(WMSTekSessions.Global.LoggedInUser))
                {
                    ...
                    Session["OrdenesSeleccionadasSimulacionWavePTL"] = newList;
        ...
        protected void btnReleasePopUp_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string errorItemUom = string.Empty;
                fullStock = true;
                ...
                if (ValidateSession(WMSTekSessions.Global.LoggedInUser))
                {
                    ...
                    else if (!string.IsNullOrEmpty(errorCustomRule))
                    {
                        this.Master.ucDialog.ShowAlert(lblTitle.Text, errorCustomRule.Trim(), string.Empty);
                    }
                    else if (!string.IsNullOrEmpty(errorItemUom))
                    {
                        this.Master.ucDialog.ShowAlert(lblTitle.Text, errorItemUom.Trim(), string.Empty);
                    }
                    else
                    {
        ...
        {
            context.SessionInfo.IdPage = "ReleaseOrderWavePTLMgr";
        ...
        private void PopulateSimulateGrid(bool refresh, bool pending, bool sort, bool init)
        {
            ...
            var simulateWavePTLWillBeQueued = SimulateWavePTLWillBeQueued();

            if (refresh)
            {
               // taskSimulationViewDTO = iDispatchingMGR.SimulateReleaseDispatchWaveNew(selectedOrdersViewDTO, context);
               // Session[WMSTekSessions.ReleaseOrderWaveMgr.TaskSimulation] = taskSimulationViewDTO;

                if (simulateWavePTLWillBeQueued)
                {
                    taskSimulationViewDTO = iDispatchingMGR.CreateSimulateReleaseToQueue(selectedOrdersViewDTO, "PIKWV_PTL", context);

                    if (!taskSimulationViewDTO.hasError() && taskSimulationViewDTO.Entities.Count > 0 && simulateWavePTLWillBeQueued)
                    {
                        ...
                        var actOrds = (GenericViewDTO<OutboundOrder>)Session["OrdenesSeleccionadasSimulacionWavePTL"]; //[WMSTekSessions.ReleaseOrderMgr.PIKOR.SelectedOrders];
        ...
        /// <summary>
        /// Valida que los items contengan presentacion de unidad
        /// </summary>
        private string ValidateItemUom()
        {
            string mensaje = string.Empty;
            ...
            foreach (TaskSimulation taskSimulation in taskSimulationViewDTO.Entities)
            {
                foreach (TaskDetailSimulation detail in taskSimulation.Details.Entities)
                {
                    if (detail.OutboundDetail.Item.ItemUom == null)
                    {
                        mensaje = lblNotExistItemUom.Text;
        ...
        protected void ReleaseOrders()
        {
            ...
            bool releaseWaveWillBeQueued = ReleaseWaveWillBeQueued();

            if (releaseWaveWillBeQueued)
                taskSimulationViewDTO = iDispatchingMGR.CreateWaveToQueue(taskInfo, selectedOrdersViewDTO, Convert.ToInt16(txtUserNbrSorting.Text), chkPendinsOrders.Checked, thePtlType.PtlTypeCode, context);
            else
               taskSimulationViewDTO = iDispatchingMGR.ReleaseDispatchWavePTL(taskInfo, thePtlType, selectedOrdersViewDTO, Convert.ToInt16(txtUserNbrSorting.Text), chkPendinsOrders.Checked, context);
        ...
        private bool SimulateWavePTLWillBeQueued()
        {
            bool willBeQueued = false;

            var value = context.CfgParameters.Where(param => param.ParameterCode == "SimulateWavePTLWillBeQueued").FirstOrDefault();
        ...
        private bool ReleaseWaveWillBeQueued()
        {
            ...
            var value = context.CfgParameters.Where(param => param.ParameterCode == "ReleaseWavePTLWillBeQueued").FirstOrDefault();
        ...
        {
            //ScriptManager.RegisterStartupScript(Page, GetType(), "gridHeader", "initializeGridDragAndDrop('OutboundOrder_GetByTrackFilter', 'ctl00_MainContent_hsVertical_leftPanel_ctl01_hsMasterDetail_topPanel_ctl01_grdMgr', 'ReleaseOrderWaveMgr');", true);
